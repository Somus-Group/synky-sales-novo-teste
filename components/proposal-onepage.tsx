...
export function ProposalOnePage({ proposal, accepted = false, accepting = false, onAccept, preview = false, actions, idPrefix = 'proposal', editable = false, selectedSlide = -1, onSelectSlide, onEditSlide, onEditSubtitle, onEditClient, onEditValue, onMoveSlide, onResizeSlide, onBackgroundSlide, backgroundPresets = [] }: OnePageProps) {
  ...
  return <article className={`relative [overflow-wrap:anywhere] [&_p]:whitespace-pre-line ${palette.page} ${preview ? 'text-[90%]' : ''}`}>
    ...
    <section id={`${idPrefix}-section-0`} onClick={() => onSelectSlide?.(0)} style={customSurface(slides[0].backgroundColor || primaryColor)} className={`relative flex ${heroHeight} flex-col justify-end overflow-hidden px-[6vw] pb-[max(6rem,7vw)] pt-[12vw] ${palette.hero} ${editable && selectedSlide === 0 ? 'ring-4 ring-[#5b7fff] ring-inset' : ''}`}>
      ...
      <div style={scaleStyle(slides[0])} className="relative z-10">
        ...
        <div className={`mt-10 grid gap-6 border-t pt-6 md:grid-cols-[1fr_auto] ${palette.heroBorder}`}><EditableText editable={editable} value={proposal.content?.subtitle || 'Uma proposta criada para transformar clareza em movimento.'} onChange={(value) => onEditSubtitle?.(value)} className="max-w-xl text-[clamp(14px,1.4vw,20px)] leading-relaxed opacity-55" /><div className="md:text-right"><span className="block text-[9px] uppercase tracking-[0.22em] opacity-40">Preparada para</span><EditableText as="strong" editable={editable} value={proposal.client} onChange={(value) => onEditClient?.(value)} className="mt-2 block text-[clamp(16px,1.7vw,24px)] font-medium" /></div></div>
      </div>
...
function NarrativeSection({ id, theme, section, index, height, accentColor, visualImage, toolbar, editable = false, selected = false, onSelect, onEdit }: { id: string; theme: ProposalTemplateId; section: ProposalSlideContent; index: number; height: string; accentColor?: string; visualImage?: ProposalVisualImage; toolbar?: ReactNode; editable?: boolean; selected?: boolean; onSelect?: () => void; onEdit?: (field: 'eyebrow' | 'title' | 'body' | 'bullets', value: string | string[]) => void }) {
  const palette = getPalette(theme);
...
function InvestmentSection({ id, theme, section, proposal, height, accentColor, toolbar, editable = false, selected = false, onSelect, onEdit, onValueChange }: { id: string; theme: ProposalTemplateId; section: ProposalSlideContent; proposal: ArtworkProposal; height: string; accentColor?: string; toolbar?: ReactNode; editable?: boolean; selected?: boolean; onSelect?: () => void; onEdit?: (field: 'eyebrow' | 'title' | 'body' | 'bullets', value: string | string[]) => void; onValueChange?: (value: number) => void }) {
...
function EditableText({ as: Tag = 'p', editable, value, suffix = '', className = '', style, onChange }: { as?: 'p' | 'h1' | 'h2' | 'strong'; editable?: boolean; value: string; suffix?: string; className?: string; style?: CSSProperties; onChange?: (value: string) => void }) {
  return <>
    <Tag
