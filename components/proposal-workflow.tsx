...
export function ProposalWorkflow({ profile, onSetup, onGenerated, onNotify, renderTemplates, initialTemplate }: Props) {
  ...
  return <div className="proposal-workflow mx-auto max-w-6xl pb-8">
    ...
    {!configured && <div className="proposal-workflow-notice mb-5 rounded-2xl border border-amber-200 bg-amber-50 p-5"><p className="text-base">Configure os dados, serviços e referências da sua empresa para começar.</p><Button className="mt-3" onClick={onSetup}>Configurar empresa</Button></div>}
    {error && <p role="alert" className="proposal-workflow-error mb-5 rounded-2xl border border-red-200 bg-red-50 p-4 text-sm leading-relaxed text-red-800">{error}</p>}
    {busy && <output aria-live="polite" className="proposal-workflow-busy mb-5 flex items-center gap-3 rounded-2xl bg-[#EDF3F0] p-4 text-sm"><Loader2 className="size-4 animate-spin" />{busy === 'briefing' ? 'Analisando o contexto e preparando as perguntas…' : busy === 'copy' ? 'Redigindo a proposta completa a partir do briefing aprovado…' : 'Aplicando o layout e salvando a proposta…'}</output>}
