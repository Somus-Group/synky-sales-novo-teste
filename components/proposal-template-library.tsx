...
export function ProposalTemplateLibrary({ onUse, onNotify }: { onUse: (value: string) => void; onNotify: (message: string) => void }) {
  ...
  return <section className={styles.library} aria-label="Biblioteca de templates de propostas">
    <div className={styles.intro}><div><span className={styles.label}>COLEÇÃO SYNKY / PROPOSTAS</span><h2>Uma boa proposta começa pela apresentação.</h2><p>Escolha um modelo para seu negócio. Abra a prévia, explore as etapas e adapte ao seu próximo cliente.</p></div><span className={styles.count}>{collectionTemplates.length}<small>modelos completos</small></span></div>
