...
function ProposalTemplateLibrary({ onUse, onNotify }: { onUse: (template: string) => void; onNotify: (message: string) => void }) {
  ...
  const [preview, setPreview] = useState<typeof proposalTemplates[number] | null>(null);
  ...
  return <><section><div className="mb-6 flex flex-col gap-4 rounded-[26px] border border-[#0B6FE8]/10 bg-[linear-gradient(135deg,#F7FBFF,#FFFFFF)] p-5 shadow-[0_12px_32px_rgba(11,111,232,0.06)] sm:flex-row sm:items-end sm:justify-between"><div><span className="inline-flex items-center gap-2 rounded-full bg-[#EAF2FF] px-3 py-1.5 text-[9px] font-semibold uppercase tracking-[0.12em] text-[#0757C8]"><LayoutTemplate className="size-3" /> Biblioteca de referências</span><h2 className="mt-4 text-xl font-semibold tracking-[-0.035em]">Compare estilos antes de começar.</h2><p className="mt-2 max-w-xl text-[11px] leading-5 text-[#60708B]">São {proposalTemplates.length} modelos pré-definidos. Clique em uma capa para abrir a prévia completa, baixe o roteiro ou aplique o modelo ao agente.</p></div><div className="flex flex-wrap gap-2">{(['Todos', ...proposalNiches] as const).map((item) => <button key={item} onClick={() => setNiche(item)} className={`rounded-full px-3 py-2 text-[10px] font-medium transition-colors ${niche === item ? 'bg-[#0B6FE8] text-white shadow-[0_8px_18px_rgba(11,111,232,0.2)]' : 'bg-white text-[#60708B] ring-1 ring-black/[0.06] hover:text-[#11244A]'}`}>{item}</button>)}</div></div><div className="grid gap-5 sm:grid-cols-2 xl:grid-cols-3">{templates.map((template) => <article key={template.id} className="group overflow-hidden rounded-[24px] border border-black/[0.06] bg-white shadow-[0_8px_24px_rgba(25,38,34,0.045)] transition-all hover:-translate-y-1 hover:shadow-[0_18px_42px_rgba(11,111,232,0.12)]"><button type="button" className="relative block aspect-[16/9] w-full overflow-hidden text-left" onClick={() => setPreview(template)} aria-label={`Ver prévia do template ${template.name}`}><ProposalThumbnail template={template.value} /><span className="absolute inset-0 grid place-items-center bg-[#0B6FE8]/0 text-[10px] font-semibold text-white opacity-0 transition-all group-hover:bg-[#0B6FE8]/60 group-hover:opacity-100 group-focus-within:bg-[#0B6FE8]/60 group-focus-within:opacity-100"><span className="rounded-full bg-white/20 px-3 py-2 backdrop-blur">Ver prévia</span></span></button><div className="p-4"><div className="flex items-start justify-between gap-3"><div><span className="text-[9px] font-semibold uppercase tracking-[0.12em] text-[#0B6FE8]">{template.niche} · {template.theme}</span><h3 className="mt-1 text-[14px] font-semibold">{template.name}</h3></div><div className="flex gap-1">{template.swatches.map((color) => <i key={color} className="size-3 rounded-full border border-black/10" style={{ backgroundColor: color }} />)}</div></div><p className="mt-2 min-h-8 text-[10px] leading-4 text-[#6E7C92]">{template.description}</p><p className="mt-3 line-clamp-2 text-[9px] leading-4 text-[#8E8E93]"><strong className="font-medium text-[#536381]">Estrutura:</strong> {template.structure}</p><div className="mt-4 grid grid-cols-3 gap-2 border-t border-black/[0.06] pt-3"><Button variant="outline" size="sm" className="h-9 rounded-xl border-black/10 px-2 text-[10px]" onClick={() => setPreview(template)}><Eye /> Prévia</Button><Button variant="outline" size="sm" className="h-9 rounded-xl border-black/10 px-2 text-[10px]" onClick={() => downloadTemplate(template)}><Download /> Baixar</Button><Button size="sm" className="h-9 rounded-xl bg-[#0B6FE8] px-2 text-[10px] text-white hover:bg-[#0757C8]" onClick={() => onUse(template.value)}><Sparkles /> Usar</Button></div></div></article>)}</div></section><TemplatePreviewDialog template={preview} onOpenChange={(open) => { if (!open) setPreview(null); }} onUse={() => { if (preview) onUse(preview.value); }} /></>;
}
