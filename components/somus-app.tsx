...
function cloneProposalContent(content: AgentResult['proposal']): AgentResult['proposal'] {
  return { ...content, company: content.company ? { ...content.company } : undefined, portfolio_images: content.portfolio_images?.map((item) => ({ ...item })), slides: content.slides.map((item) => ({ ...item, bullets: [...item.bullets] })) };
}
