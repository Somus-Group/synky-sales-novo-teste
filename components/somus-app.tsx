...
function ProposalEditor({ proposal, onBack, onNotify }: { proposal: Proposal; onBack: () => void; onNotify: (message: string) => void }) {
  ...
  function moveSelectedSection(direction: -1 | 1) {
    if (slide <= 0 || slide >= content.slides.length - 1) { onNotify('A capa e o encerramento permanecem nas extremidades'); return; }
    const nextIndex = slide + direction;
    if (nextIndex <= 0 || nextIndex >= content.slides.length - 1) return;
    rememberChange();
    setContent((current) => {
      const nextSlides = [...current.slides];
      [nextSlides[slide], nextSlides[nextIndex]] = [nextSlides[nextIndex], nextSlides[slide]];
  ...
  function resizeSelectedSection(delta: number) {
    rememberChange();
    setContent((current) => ({ ...current, slides: current.slides.map((item, index) => index === slide ? { ...item, fontScale: Math.max(70, Math.min(120, (item.fontScale || 100) + delta)) } : item) }));
  }
  function addSection() {
    rememberChange();
