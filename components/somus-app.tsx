...
function Team({ members, onAdd }: { members: Member[]; onAdd: () => void }) {
  ...
  return <><PageTitle kicker="Workspace" title="Equipe" description="Pessoas, acessos e responsabilidades do seu escritório." actions={<Button onClick={onAdd} className="h-11 rounded-2xl bg-[#0B6FE8] px-5 text-white shadow-[0_10px_24px_rgba(11,111,232,0.22)] hover:bg-[#0757C8]"><UserPlus /> Adicionar usuário</Button>} />
    <section className="grid gap-4 xl:grid-cols-[1.45fr_0.7fr_0.7fr]">
      <article className="relative overflow-hidden rounded-[28px] bg-[linear-gradient(125deg,#113564_0%,#0B6FE8_56%,#3A8EF2_100%)] p-6 text-white shadow-[0_18px_40px_rgba(11,111,232,0.18)] sm:p-7"><div className="relative z-10 max-w-md"><span className="inline-flex items-center gap-2 rounded-full bg-white/15 px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.12em] text-white/80"><Users2 className="size-3.5" /> Organização do time</span><h2 className="mt-5 text-2xl font-semibold tracking-[-0.045em] sm:text-[28px]">Uma equipe alinhada deixa o trabalho fluir.</h2><p className="mt-3 max-w-sm text-[12px] leading-5 text-white/72">Centralize quem participa do workspace e mantenha os acessos organizados em um só lugar.</p><div className="mt-6 flex items-center gap-4"><div className="flex -space-x-2">{members.slice(0, 4).map((member, index) => <span key={member.id} className={`grid size-9 place-items-center rounded-full border-2 border-[#1777E6] text-[10px] font-semibold ${index % 2 ? 'bg-[#D7E9FF] text-[#0B5DBB]' : 'bg-[#E9FFF4] text-[#17634C]'}`}>{member.name.split(' ').map((part) => part[0]).join('').slice(0, 2).toUpperCase()}</span>)}{members.length > 4 && <span className="grid size-9 place-items-center rounded-full border-2 border-[#1777E6] bg-white/15 text-[9px] font-semibold">+{members.length - 4}</span>}</div><span className="text-[11px] text-white/72">{active === members.length ? 'Todos os acessos estão ativos' : `${active} de ${members.length} acessos ativos`}</span></div></div><div className="absolute -right-12 -top-16 size-64 rounded-full border-[28px] border-white/10" /><div className="absolute -bottom-24 right-20 size-48 rounded-full bg-[#90C7FF]/25 blur-3xl" /></article>
      <article className="rounded-[28px] border border-[#0B6FE8]/10 bg-white p-5 shadow-[0_10px_30px_rgba(11,111,232,0.06)]"><span className="grid size-10 place-items-center rounded-2xl bg-[#EAF2FF] text-[#0B6FE8]"><Users2 className="size-5" /></span><strong className="mt-7 block text-[34px] font-semibold tracking-[-0.055em]">{members.length}</strong><p className="mt-1 text-[11px] font-medium text-[#41506B]">{members.length === 1 ? 'pessoa no workspace' : 'pessoas no workspace'}</p><div className="mt-5 h-1.5 overflow-hidden rounded-full bg-[#EAF2FF]"><div className="h-full rounded-full bg-[#0B6FE8]" style={{ width: `${Math.min(100, Math.max(12, members.length * 14))}%` }} /></div></article>
      <article className="rounded-[28px] border border-[#20A37A]/15 bg-[linear-gradient(145deg,#ECFBF4,#F8FFFC)] p-5 shadow-[0_10px_30px_rgba(32,163,122,0.06)]"><span className="grid size-10 place-items-center rounded-2xl bg-[#D8F3E5] text-[#157352]"><ShieldCheck className="size-5" /></span><strong className="mt-7 block text-[34px] font-semibold tracking-[-0.055em]">{active}</strong><p className="mt-1 text-[11px] font-medium text-[#3B6B5A]">{active === 1 ? 'acesso ativo agora' : 'acessos ativos agora'}</p><span className={`mt-5 inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-[10px] font-medium ${pending ? 'bg-[#FFF2E3] text-[#A46024]' : 'bg-[#E6F7EF] text-[#277157]'}`}><i className="size-1.5 rounded-full bg-current" />{pending ? `${pending} pendente${pending > 1 ? 's' : ''}` : 'Tudo em ordem'}</span></article>
    </section>
    <section className="mt-5 grid gap-5 xl:grid-cols-[minmax(0,1fr)_300px]">
      <article className="overflow-hidden rounded-[28px] border border-black/[0.06] bg-white shadow-[0_10px_34px_rgba(25,38,34,0.05)]"><div className="flex flex-col gap-4 border-b border-black/[0.055] px-5 py-5 sm:flex-row sm:items-center sm:justify-between md:px-6"><div><span className="text-[9px] font-semibold uppercase tracking-[0.14em] text-[#5A78A2]">Gestão de acesso</span><h2 className="mt-1 text-[17px] font-semibold tracking-[-0.03em]">Pessoas do workspace</h2><p className="mt-1 text-[11px] text-[#6E7C92]">Função, status e acesso de cada pessoa da sua operação.</p></div><Button onClick={onAdd} variant="outline" className="h-10 rounded-xl border-[#0B6FE8]/20 bg-white px-4 text-[#0B6FE8] hover:bg-[#EAF2FF] hover:text-[#0757C8]"><Plus /> Adicionar</Button></div><div className="hidden grid-cols-[minmax(220px,1.4fr)_0.8fr_0.7fr_auto] gap-4 border-b border-black/[0.055] bg-[#F8FBFF] px-6 py-3 text-[9px] font-semibold uppercase tracking-[0.12em] text-[#7890AF] md:grid"><span>Pessoa</span><span>Função</span><span>Status</span><span>Acesso</span></div><div className="divide-y divide-black/[0.055]">{members.map((member, index) => <div key={member.id} className="group grid items-center gap-4 px-5 py-4 transition-colors hover:bg-[#F8FBFF] md:grid-cols-[minmax(220px,1.4fr)_0.8fr_0.7fr_auto] md:px-6"><div className="flex min-w-0 items-center gap-3"><span className={`grid size-11 shrink-0 place-items-center rounded-2xl text-[11px] font-semibold ring-4 ring-white ${index % 3 === 0 ? 'bg-[#EAF6F0] text-[#277157]' : index % 3 === 1 ? 'bg-[#EEF0FF] text-[#5164C9]' : 'bg-[#FFF2E3] text-[#A46024]'}`}>{member.name.split(' ').map((part) => part[0]).join('').slice(0, 2).toUpperCase()}</span><span className="min-w-0"><strong className="block truncate text-[13px] font-semibold">{member.name}{member.isCurrent ? <em className="ml-2 rounded-full bg-[#EAF2FF] px-2 py-0.5 text-[9px] font-medium not-italic text-[#0B6FE8]">Você</em> : null}</strong><span className="mt-1 flex min-w-0 items-center gap-1.5 truncate text-[10px] text-[#7B8799]"><Mail className="size-3 shrink-0" />{member.email}</span></span></div><span className="w-fit rounded-full bg-[#F2F5F9] px-2.5 py-1 text-[10px] font-medium text-[#536381]">{member.role}</span><span className={`flex items-center gap-1.5 text-[10px] font-medium ${member.status === 'Ativo' ? 'text-[#277157]' : 'text-[#9A6B37]'}`}><i className="size-1.5 rounded-full bg-current" />{member.status}</span><span className="w-fit rounded-full border border-[#DCE6F2] bg-white px-2.5 py-1 text-[9px] text-[#6E7C92]">{member.status === 'Ativo' ? 'Acesso confirmado' : 'Aguardando entrada'}</span></div>)}{members.length === 0 && <div className="px-6 py-16 text-center"><span className="mx-auto grid size-12 place-items-center rounded-2xl bg-[#EAF2FF] text-[#0B6FE8]"><Users2 className="size-5" /></span><strong className="mt-4 block text-sm">Sua equipe começa por aqui</strong><p className="mx-auto mt-2 max-w-xs text-[11px] leading-5 text-[#7B8799]">Adicione a primeira pessoa para organizar acessos e responsabilidades.</p><Button onClick={onAdd} className="mt-5 rounded-xl bg-[#0B6FE8] text-white"><Plus /> Adicionar usuário</Button></div>}</div></article>
      <aside className="space-y-5"><section className="rounded-[28px] border border-[#0B6FE8]/10 bg-[#F4F8FF] p-5"><div className="flex items-center justify-between"><span className="grid size-10 place-items-center rounded-2xl bg-white text-[#0B6FE8] shadow-sm"><ShieldCheck className="size-5" /></span><span className="rounded-full bg-[#DCEBFF] px-2.5 py-1 text-[9px] font-semibold text-[#0B6FE8]">Seguro</span></div><h2 className="mt-5 text-[17px] font-semibold tracking-[-0.03em] text-[#113564]">Visão de acesso</h2><p className="mt-2 text-[11px] leading-5 text-[#5D7190]">Acompanhe rapidamente a disponibilidade da sua equipe no workspace.</p><div className="mt-5 space-y-3 border-t border-[#0B6FE8]/10 pt-4"><div className="flex items-center justify-between text-[11px]"><span className="text-[#5D7190]">Ativos</span><strong className="text-[#113564]">{active} {active === 1 ? 'pessoa' : 'pessoas'}</strong></div><div className="flex items-center justify-between text-[11px]"><span className="text-[#5D7190]">Pendentes</span><strong className="text-[#113564]">{pending}</strong></div></div></section><section className="relative overflow-hidden rounded-[28px] bg-[#172A25] p-5 text-white"><Sparkles className="size-5 text-[#F4C981]" /><p className="mt-5 text-[14px] font-medium leading-6">“Talento individual cria ideias. Uma equipe alinhada transforma ideias em legado.”</p><span className="mt-5 block text-[10px] text-white/45">Cultive contexto, autonomia e clareza.</span><div className="absolute -bottom-10 -right-8 size-32 rounded-full bg-[#20A37A]/25 blur-2xl" /></section></aside>
    </section>
  </>;
}
...
function OpportunityDialog({ open, onOpenChange, opportunity, defaultStage, labels, saving, error, onSave, onDelete }: { open: boolean; onOpenChange: (open: boolean) => void; opportunity: Opportunity | null; defaultStage: Stage; labels: PipelineLabels; saving: boolean; error: string; onSave: (input: OpportunityInput) => Promise<boolean>; onDelete: (id: number) => Promise<boolean> }) {
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}>
    <DialogContent className="flex h-[calc(100dvh-1rem)] max-h-none max-w-2xl flex-col gap-0 overflow-hidden rounded-[28px] p-0 sm:h-[min(88dvh,820px)] sm:max-w-3xl lg:max-w-4xl">
      <DialogHeader className="shrink-0 border-b border-[#17382e]/[0.08] bg-[linear-gradient(115deg,#f8fbfa_0%,#ffffff_55%,#eef5f1_100%)] px-6 pb-5 pt-6 pr-16 sm:px-8 sm:pb-6 sm:pt-7">
        <span className="mb-3 grid size-12 place-items-center rounded-[18px] bg-[#17382e] text-white shadow-[0_12px_24px_rgba(23,56,46,0.18)]">{opportunity ? <Pencil className="size-5" /> : <Plus className="size-5" />}</span>
        <DialogTitle className="text-2xl font-semibold tracking-[-0.04em] text-[#142b24]">{opportunity ? 'Editar oportunidade' : 'Nova oportunidade'}</DialogTitle>
        <DialogDescription className="max-w-xl text-[13px] leading-5 text-[#64736d]">Organize os dados comerciais, a etapa, as tags e os campos específicos deste negócio.</DialogDescription>
      </DialogHeader>
      <form onSubmit={submit} className="flex min-h-0 flex-1 flex-col">
        <div className="min-h-0 flex-1 space-y-6 overflow-y-auto px-6 py-6 overscroll-contain sm:px-8 sm:py-7">
          <div className="grid gap-x-5 gap-y-5 md:grid-cols-2"><Field label="Cliente"><Input required value={form.client} onChange={(event) => update('client', event.target.value)} className="h-12 rounded-2xl border-[#17382e]/10 bg-[#fcfdfc] px-4 shadow-sm" placeholder="Nome do cliente" /></Field><Field label="Oportunidade ou projeto"><Input required value={form.project} onChange={(event) => update('project', event.target.value)} className="h-12 rounded-2xl border-[#17382e]/10 bg-[#fcfdfc] px-4 shadow-sm" placeholder="Ex.: Implantação comercial" /></Field></div>
          <div className="grid gap-x-5 gap-y-5 md:grid-cols-2"><Field label="Valor estimado"><Input required min="0" type="number" value={form.value || ''} onChange={(event) => update('value', Number(event.target.value))} className="h-12 rounded-2xl border-[#17382e]/10 bg-[#fcfdfc] px-4 shadow-sm" /></Field><Field label="Etapa"><AppSelect value={form.stage} onValueChange={(value) => update('stage', value as Stage)} ariaLabel="Etapa da oportunidade" options={(['Novo contato', 'Diagnóstico', 'Proposta enviada', 'Negociação'] as Stage[]).map((value) => ({ value, label: labels.stages[value] }))} /></Field></div>
          <div className="grid gap-x-5 gap-y-5 md:grid-cols-2"><Field label="Próxima ação"><Input value={form.due} onChange={(event) => update('due', event.target.value)} className="h-12 rounded-2xl border-[#17382e]/10 bg-[#fcfdfc] px-4 shadow-sm" placeholder="Ex.: Retornar sexta, 14h" /></Field><Field label="Origem"><Input value={form.source} onChange={(event) => update('source', event.target.value)} className="h-12 rounded-2xl border-[#17382e]/10 bg-[#fcfdfc] px-4 shadow-sm" placeholder="Ex.: Indicação, site, evento" /></Field></div>
          <Field label="Tags"><div className="relative"><Tag className="absolute left-4 top-1/2 size-4 -translate-y-1/2 text-[#719087]" /><Input value={tagsText} onChange={(event) => setTagsText(event.target.value)} className="h-12 rounded-2xl border-[#17382e]/10 bg-[#fcfdfc] pl-11 pr-4 shadow-sm" placeholder="Prioridade, alto valor, follow-up" /></div></Field>

          <section className="rounded-[22px] border border-[#17382e]/[0.08] bg-[#f4f8f6] p-5 sm:p-6"><div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div><strong className="text-[13px] text-[#17382e]">Campos personalizados</strong><p className="mt-1 max-w-md text-[11px] leading-4 text-[#718078]">Adicione qualquer informação importante para sua operação.</p></div><Button type="button" variant="outline" size="sm" className="h-10 self-start rounded-xl border-[#17382e]/10 bg-white px-4 shadow-sm hover:bg-[#eaf2ee]" onClick={() => setFields((items) => [...items, { key: '', value: '' }])}><Plus /> Adicionar campo</Button></div>
        ...
        <DialogFooter className="m-0 shrink-0 rounded-none rounded-b-[28px] border-t border-[#17382e]/[0.07] bg-white px-6 py-5 pb-[max(1.25rem,env(safe-area-inset-bottom))] shadow-[0_-12px_35px_rgba(23,56,46,0.045)] sm:flex-col-reverse sm:px-8 md:flex-row md:items-center md:justify-between">
          ...
          <div className="grid w-full grid-cols-2 gap-2 md:ml-auto md:flex md:w-auto">
            ...
            <Button disabled={saving || deleting} type="submit" className="h-11 w-full whitespace-nowrap rounded-xl bg-[#17382e] px-6 text-white shadow-[0_10px_22px_rgba(23,56,46,0.2)] hover:bg-[#0f2720] md:w-auto">{saving ? 'Salvando...' : opportunity ? 'Salvar alterações' : 'Criar oportunidade'}</Button>
          </div>
        </DialogFooter>
      </form>
    </DialogContent>
