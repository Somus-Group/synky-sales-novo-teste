...
type AgentResult = {
  ...
  proposal: { budget_pending?: boolean; brand_name: string; logo_url?: string; hero_image_url?: string; primary_color?: string; secondary_color?: string; company?: { description?: string; website?: string; email?: string; phone?: string; address?: string; instagram?: string }; portfolio_images?: Array<{ url: string; name: string; caption: string }>; title: string; subtitle: string; slides: Array<{ type: string; eyebrow: string; title: string; body: string; bullets: string[]; backgroundColor?: string; fontScale?: number; elementStyles?: Record<string, { x?: number; y?: number; scale?: number; width?: number; noWrap?: boolean }> }> };
};
type ProposalEditorSnapshot = { slide: number; title: string; client: string; value: number; template: string; content: AgentResult['proposal'] };
...
export function SomusApp({ userName, userEmail, initialView = 'overview' }: { userName: string; userEmail: string; initialView?: 'overview' | 'zero' }) {
  ...
  const sidebarCollapsed = (view === 'studio' || view === 'conversation' || view === 'zero') && !mobileNav ? !studioNavOpen : sidebarPreference;
    ...
    <div className={`sales-app min-h-screen transition-colors ${theme === 'dark' ? 'theme-dark bg-[#17191E] text-[#EAF2FF]' : 'bg-[#F2F7FF] text-[#11244A]'}`}>
      <aside className={`sales-sidebar fixed inset-y-0 left-0 z-50 w-[256px] border-r p-3 backdrop-blur-2xl transition-[width,padding,transform] duration-300 ease-out lg:translate-x-0 ${sidebarCollapsed ? 'sidebar-collapsed' : ''} ${theme === 'dark' ? 'text-[#EAF2FF]' : ''} ${mobileNav ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="flex h-[72px] items-center justify-between px-2">
          ...
          <Button variant="ghost" size="icon-sm" className="sidebar-collapse-toggle hidden lg:grid" onClick={() => view === 'studio' || view === 'conversation' || view === 'zero' ? setStudioNavOpen((value) => !value) : setSidebarCollapsed((value) => !value)} aria-label={sidebarCollapsed ? 'Expandir menu' : 'Recolher menu'} title={sidebarCollapsed ? 'Expandir menu' : 'Recolher menu'}>{sidebarCollapsed ? <PanelLeftOpen /> : <PanelLeftClose />}</Button>
      ...
      <div className={`transition-[padding] duration-300 ease-out ${sidebarCollapsed ? 'lg:pl-[72px]' : 'lg:pl-[256px]'}`}>
        ...
        <main data-view={view} className={view === 'studio' || view === 'conversation' || view === 'zero' ? 'sales-content w-full' : 'sales-content mx-auto max-w-[1440px] px-4 py-6 sm:px-5 md:px-8 md:py-10'}>
...
function Overview({ opportunities, proposals, pipelineValue, onPipeline, onProposal, onProposals, onOpenProposal }: { opportunities: Opportunity[]; proposals: Proposal[]; pipelineValue: number; onPipeline: () => void; onProposal: () => void; onProposals: () => void; onOpenProposal: (proposal: Proposal) => void }) {
  ...
  return <div className="overview-root">
    <section className="overview-hero relative mb-5 overflow-hidden rounded-[26px] border border-[#0B6FE8]/15 bg-white px-5 py-6 shadow-[0_12px_35px_rgba(11,111,232,0.08)] sm:px-7 sm:py-7">
      ...
      <div className="overview-heading-row relative flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
        <div className="overview-heading-copy min-w-0">
    ...
    <section className="mt-5 grid gap-5 xl:grid-cols-[1.35fr_0.85fr]">
      ...
      <article className="rounded-[26px] border border-black/[0.06] bg-white p-5 shadow-[0_8px_30px_rgba(25,38,34,0.04)] md:p-6"><SectionHeader title="Funil de oportunidades" subtitle="Valor por etapa" action="Abrir CRM" onAction={onPipeline} /><div className="overview-funnel-list mt-7 space-y-5">{stages.map(({ stage, items, total }, index) => <button onClick={onPipeline} key={stage} className="block w-full text-left"><div className="mb-2 flex items-center justify-between"><span className="flex items-center gap-2 text-[11px] font-medium"><i className="size-2 rounded-full" style={{ background: stageColors[stage] }} />{stage}</span><span className="text-[10px] text-[#8e8e93]">{items.length} · {money.format(total)}</span></div><div className="h-2.5 overflow-hidden rounded-full bg-[#f0f0f3]"><div className="overview-funnel-bar h-full rounded-full" style={{ width: `${Math.max(24, 100 - index * 19)}%`, background: stageColors[stage], animationDelay: `${220 + index * 110}ms` }} /></div></button>)}</div><div className="overview-funnel-callout mt-7 flex items-center gap-3 rounded-2xl border border-[#D7E7FA] bg-[#F3F8FF] p-4"><span className="grid size-9 place-items-center rounded-xl bg-white text-[#075FC5] shadow-sm"><TrendingUp className="size-4" /></span><p className="text-[10px] leading-4 text-[#527092]"><strong className="block text-[11px] text-[#163C68]">Seu melhor movimento agora</strong>Priorize as propostas já visualizadas: elas têm maior intenção de compra.</p></div></article>
    </section>
...
function VisualStat({ color, icon: Icon, label, value, detail }: { color: 'violet' | 'green' | 'orange' | 'rose'; icon: typeof Users2; label: string; value: string; detail: string }) {
  ...
  const trends = { violet: '4,31 18,26 31,28 45,17 59,21 73,8 86,12', green: '4,32 17,23 29,28 43,14 56,20 70,7 86,11', orange: '4,33 18,27 32,29 46,20 58,22 71,8 86,12', rose: '4,31 17,25 31,28 45,18 58,24 72,9 86,13' } as const;
  return <article className={`overview-stat overview-stat-${color} relative overflow-hidden rounded-[24px] p-5 ${palette[0]}`}><span aria-hidden="true" className="overview-stat-accent absolute inset-x-0 top-0 h-1" /><div className={`grid size-9 place-items-center rounded-xl ${palette[1]}`}><Icon className="size-4" /></div><p className="mt-5 text-[10px] font-medium text-black/45">{label}</p><strong className="mt-1.5 block text-[26px] font-semibold tracking-[-0.04em]">{value}</strong><div className="overview-stat-footer mt-4 flex items-end justify-between gap-3"><p className="text-[10px] text-black/38">{detail}</p><svg aria-hidden="true" viewBox="0 0 90 40" className="overview-stat-sparkline h-10 w-[90px] shrink-0"><path d="M4 36 H86" className="overview-stat-baseline" /><polyline points={trends[color]} fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" /><circle cx="86" cy={color === 'green' ? '11' : color === 'orange' ? '12' : color === 'rose' ? '13' : '12'} r="3" fill="currentColor" /></svg></div></article>;
}
...
function cloneProposalContent(content: AgentResult['proposal']): AgentResult['proposal'] {
  return { ...content, company: content.company ? { ...content.company } : undefined, portfolio_images: content.portfolio_images?.map((item) => ({ ...item })), slides: content.slides.map((item) => ({ ...item, bullets: [...item.bullets], elementStyles: item.elementStyles ? Object.fromEntries(Object.entries(item.elementStyles).map(([key, value]) => [key, { ...value }])) : undefined })) };
}
...
function ProposalEditor({ proposal, onBack, onNotify }: { proposal: Proposal; onBack: () => void; onNotify: (message: string) => void }) {
  ...
  const [undoStack, setUndoStack] = useState<ProposalEditorSnapshot[]>([]);
  ...
  function rememberChange(token?: string) {
    if (token && editSessionRef.current === token) return;
    if (token) editSessionRef.current = token;
    const snapshot = { slide, title, client, value, template, content: cloneProposalContent(content) };
    setUndoStack((items) => [...items.slice(-24), snapshot]);
  }
  function finishChange(token: string) {
    if (editSessionRef.current === token) editSessionRef.current = '';
  }
  function undoChange() {
    ...
    setSlide(Math.min(previous.slide, Math.max(0, previous.content.slides.length - 1)));
    setTitle(previous.title);
    setClient(previous.client);
    setValue(previous.value);
    setTemplate(previous.template);
    setContent(cloneProposalContent(previous.content));
    onNotify('Alteração desfeita');
  }
  ...
  function updateSlide(index: number, field: 'eyebrow' | 'title' | 'body' | 'bullets', next: string | string[]) {
    rememberChange();
    setSlide(index);
    setContent((current) => ({ ...current, slides: current.slides.map((item, itemIndex) => itemIndex === index ? { ...item, [field]: next } : item) }));
  }
  function moveSlide(index: number, direction: -1 | 1) {
    if (index <= 0 || index >= content.slides.length - 1) { onNotify('A capa e o encerramento permanecem nas extremidades'); return; }
    const nextIndex = index + direction;
    if (nextIndex <= 0 || nextIndex >= content.slides.length - 1) return;
    rememberChange();
    setContent((current) => {
    ...
    setSlide(nextIndex);
    window.requestAnimationFrame(() => document.getElementById(`editor-proposal-section-${nextIndex}`)?.scrollIntoView({ behavior: 'smooth', block: 'start' }));
  }
  function moveSelectedSection(direction: -1 | 1) {
    moveSlide(slide, direction);
  }
  function resizeSlide(index: number, delta: number) {
    rememberChange();
    setSlide(index);
    setContent((current) => ({ ...current, slides: current.slides.map((item, itemIndex) => itemIndex === index ? { ...item, fontScale: Math.max(70, Math.min(120, (item.fontScale || 100) + delta)) } : item) }));
  }
  function resizeSelectedSection(delta: number) {
    resizeSlide(slide, delta);
  }
  function updateSlideBackground(index: number, color: string) {
    rememberChange();
    setSlide(index);
    setContent((current) => ({ ...current, slides: current.slides.map((item, itemIndex) => itemIndex === index ? { ...item, backgroundColor: color } : item) }));
  }
  function updateElementStyle(index: number, key: string, patch: { x?: number; y?: number; scale?: number; width?: number; noWrap?: boolean }) {
    rememberChange();
    setSlide(index);
    setContent((current) => ({ ...current, slides: current.slides.map((item, itemIndex) => {
      if (itemIndex !== index) return item;
      const currentStyle = item.elementStyles?.[key] || {};
      return { ...item, elementStyles: { ...(item.elementStyles || {}), [key]: { ...currentStyle, ...patch } } };
    }) }));
  }
  function updateCanvasClient(next: string) {
    rememberChange();
    setClient(next);
  }
  function updateCanvasValue(next: number) {
    rememberChange();
    setValue(next);
    setContent((current) => ({ ...current, budget_pending: false }));
  }
  function addSection() {
    rememberChange();
  ...
  return <div className="flex h-[100dvh] flex-col overflow-hidden bg-[#e9e9ec] text-[#1d1d1f]">
    ...
    <div className="grid min-h-0 flex-1 grid-cols-[64px_minmax(0,1fr)] xl:grid-cols-[64px_190px_minmax(0,1fr)_310px]">
      ...
      <main className="relative flex min-w-0 flex-col overflow-hidden"><div className="flex h-12 shrink-0 items-center gap-1 overflow-x-auto border-b border-black/[0.07] bg-white/70 px-3 backdrop-blur"><span className="mr-2 hidden shrink-0 text-[10px] font-semibold uppercase tracking-[0.1em] text-[#8E8E93] sm:inline">Visualização do site</span><Button onClick={() => onNotify('Clique no texto para editar, esticar, mover, diminuir e controlar quebra de linha.')} variant="ghost" size="sm" className="rounded-lg text-[11px]">Editar direto</Button><Button onClick={() => onNotify('Peso Semibold selecionado')} variant="ghost" size="sm" className="rounded-lg text-[11px]">Semibold</Button><span className="mx-1 h-5 w-px shrink-0 bg-black/10" /><Button aria-label="Mais opções de texto" onClick={() => onNotify('Mais opções de texto')} variant="ghost" size="icon-sm"><MoreHorizontal /></Button></div><div className="flex-1 overflow-y-auto overflow-x-hidden p-3 sm:p-5 md:p-8"><div className="mx-auto w-full"><div style={{ width: `${10000 / zoom}%`, transform: `scale(${zoom / 100})`, transformOrigin: 'top left' }} className="overflow-hidden rounded-xl bg-white shadow-[0_25px_70px_rgba(0,0,0,0.18)]"><ProposalOnePage preview editable selectedSlide={slide} onSelectSlide={setSlide} onEditSlide={updateSlide} onEditSubtitle={(value) => { rememberChange(); setContent((current) => ({ ...current, subtitle: value })); }} onEditClient={updateCanvasClient} onEditValue={updateCanvasValue} onMoveSlide={moveSlide} onResizeSlide={resizeSlide} onBackgroundSlide={updateSlideBackground} onEditElementStyle={updateElementStyle} backgroundPresets={backgroundPresets} idPrefix="editor-proposal" proposal={{ ...proposal, client, project: title, value, template, content }} /></div><div className="sticky bottom-4 mx-auto mt-5 flex w-fit items-center gap-1 rounded-xl border border-black/[0.08] bg-white p-1 shadow-lg"><Button variant="ghost" size="icon-sm" onClick={() => setZoom((current) => Math.max(40, current - 10))}><ZoomOut /></Button><span className="w-12 text-center text-[10px] text-[#6e6e73]">{zoom}%</span><Button variant="ghost" size="icon-sm" onClick={() => setZoom((current) => Math.min(100, current + 10))}><ZoomIn /></Button></div></div></div></main>
      <aside className="hidden overflow-y-auto border-l border-black/[0.08] bg-white p-5 xl:block"><div className="flex items-start justify-between gap-3"><div><p className="text-[10px] font-semibold uppercase tracking-[0.1em] text-[#8e8e93]">Editar seção</p><h2 className="mt-2 text-lg font-semibold tracking-[-0.02em]">{slides[slide]}</h2></div>{slide > 0 && slide < content.slides.length - 1 && <Button aria-label="Excluir seção" variant="ghost" size="icon-sm" className="text-[#C03F32]" onClick={deleteSection}><Trash2 /></Button>}</div><div className="mt-5 grid gap-3 rounded-2xl border border-black/[0.06] bg-[#F7F7F9] p-3"><div><p className="text-[10px] font-semibold uppercase tracking-[0.1em] text-[#8e8e93]">Posição manual</p><div className="mt-2 grid grid-cols-2 gap-2"><Button variant="outline" size="sm" disabled={slide <= 1 || slide >= content.slides.length - 1} className="rounded-xl bg-white" onClick={() => moveSelectedSection(-1)}><ArrowUp />Subir</Button><Button variant="outline" size="sm" disabled={slide <= 0 || slide >= content.slides.length - 2} className="rounded-xl bg-white" onClick={() => moveSelectedSection(1)}><ArrowDown />Descer</Button></div></div><div><p className="text-[10px] font-semibold uppercase tracking-[0.1em] text-[#8e8e93]">Tamanho da seção</p><div className="mt-2 grid grid-cols-[1fr_auto_1fr] items-center gap-2"><Button variant="outline" size="sm" className="rounded-xl bg-white" onClick={() => resizeSelectedSection(-5)}>Diminuir</Button><span className="text-center text-[10px] text-[#6e6e73]">{section?.fontScale || 100}%</span><Button variant="outline" size="sm" className="rounded-xl bg-white" onClick={() => resizeSelectedSection(5)}>Aumentar</Button></div></div><p className="text-[9px] leading-4 text-[#8E8E93]">Você também pode clicar no texto dentro da proposta e editar diretamente no preview.</p></div><div className="mt-6 space-y-5"><Field label="Nome interno da proposta"><Input value={title} onBlur={() => finishChange('proposal-title')} onChange={(event) => { rememberChange('proposal-title'); setTitle(event.target.value); }} className="h-10 rounded-xl border-black/10 bg-[#f7f7f9]" /></Field><Field label="Cliente"><Input value={client} onBlur={() => finishChange('proposal-client')} onChange={(event) => { rememberChange('proposal-client'); setClient(event.target.value); }} className="h-10 rounded-xl border-black/10 bg-[#f7f7f9]" /></Field>{section?.type === 'investment' && <Field label="Investimento"><Input value={value} onBlur={() => finishChange('proposal-value')} onChange={(event) => { rememberChange('proposal-value'); setValue(Number(event.target.value)); setContent(current => ({ ...current, budget_pending: false })); }} type="number" className="h-10 rounded-xl border-black/10 bg-[#f7f7f9]" /></Field>}<Field label="Rótulo da seção"><Input value={section?.eyebrow || ''} onBlur={() => finishChange(`section-eyebrow-${slide}`)} onChange={(event) => { rememberChange(`section-eyebrow-${slide}`); updateSection('eyebrow', event.target.value); }} className="h-10 rounded-xl border-black/10 bg-[#f7f7f9]" /></Field><Field label="Título da seção"><Input value={section?.title || ''} onBlur={() => finishChange(`section-title-${slide}`)} onChange={(event) => { rememberChange(`section-title-${slide}`); updateSection('title', event.target.value); }} className="h-10 rounded-xl border-black/10 bg-[#f7f7f9]" /></Field><Field label="Texto de apoio"><textarea className="min-h-28 w-full resize-none rounded-xl border border-black/10 bg-[#f7f7f9] p-3 text-xs leading-5 outline-none focus:ring-2 focus:ring-black/10" value={slide === 0 ? content.subtitle : section?.body || ''} onBlur={() => finishChange(slide === 0 ? 'proposal-subtitle' : `section-body-${slide}`)} onChange={(event) => { rememberChange(slide === 0 ? 'proposal-subtitle' : `section-body-${slide}`); if (slide === 0) setContent((current) => ({ ...current, subtitle: event.target.value })); else updateSection('body', event.target.value); }} /></Field>{slide > 0 && slide < content.slides.length - 1 && <Field label="Tópicos — um por linha"><textarea className="min-h-24 w-full resize-none rounded-xl border border-black/10 bg-[#f7f7f9] p-3 text-xs leading-5 outline-none focus:ring-2 focus:ring-black/10" value={(section?.bullets || []).join('\n')} onBlur={() => finishChange(`section-bullets-${slide}`)} onChange={(event) => { rememberChange(`section-bullets-${slide}`); updateSection('bullets', event.target.value.split('\n').filter(Boolean)); }} /></Field>}<Button variant="outline" className="h-10 w-full rounded-xl border-black/10 bg-white" onClick={() => onNotify('Texto preparado para refinamento com IA')}><Sparkles /> Refinar com IA</Button></div><div className="mt-8 border-t border-black/[0.07] pt-5"><div className="flex items-center gap-2"><Palette className="size-3.5 text-[#8E8E93]" /><p className="text-[10px] font-semibold uppercase tracking-[0.1em] text-[#8e8e93]">Fundo da seção</p></div><div className="mt-3 flex flex-wrap items-center gap-2"><button onClick={() => { rememberChange(); updateSection('backgroundColor', ''); }} className={`grid size-8 place-items-center rounded-full border bg-white text-[8px] text-[#8E8E93] ${!section?.backgroundColor ? 'ring-2 ring-black ring-offset-2' : 'border-black/10'}`} aria-label="Usar fundo do layout">Auto</button>{backgroundPresets.map((color) => <button key={color} onClick={() => { rememberChange(); updateSection('backgroundColor', color); }} className={`size-8 rounded-full border border-black/10 ${section?.backgroundColor === color ? 'ring-2 ring-black ring-offset-2' : ''}`} style={{ backgroundColor: color }} aria-label={`Aplicar fundo ${color}`} />)}<NativeColorInput value={section?.backgroundColor || '#F7F1E8'} onChange={(color) => { rememberChange(`section-background-${slide}`); updateSection('backgroundColor', color); }} ariaLabel="Abrir seletor de cor personalizada da seção" /></div><p className="mt-3 text-[9px] leading-4 text-[#8E8E93]">A cor é aplicada apenas nesta seção e permanece no link compartilhado.</p></div><div className="mt-8 border-t border-black/[0.07] pt-5"><p className="mb-3 text-[10px] font-semibold uppercase tracking-[0.1em] text-[#8e8e93]">Identidade visual</p><div className="space-y-2">{proposalTemplates.map((option) => <button key={option.id} onClick={() => { rememberChange(); setTemplate(option.value); onNotify(`${option.name} aplicado à proposta`); }} className={`flex w-full items-center gap-3 rounded-xl border p-2 text-left transition-all ${template === option.value ? 'border-black bg-black text-white' : 'border-black/[0.07] bg-[#F7F7F9] hover:border-black/20'}`}><span className="h-12 w-20 shrink-0 overflow-hidden rounded-lg"><ProposalThumbnail template={option.value} /></span><span className="min-w-0"><strong className="block text-[10px]">{option.name}</strong><span className={`mt-0.5 block truncate text-[8px] ${template === option.value ? 'text-white/45' : 'text-[#8E8E93]'}`}>{option.description}</span></span></button>)}</div></div></aside>
    </div>
  </div>;
}
